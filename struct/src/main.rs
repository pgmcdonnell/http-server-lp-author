...
fn main() {
  ...
  v.push(g);
  let g : Greeting = Greeting { lang: Lang::Scottish, message: String::from("Alright WasmEdge!") };
  v.push(g);
  let g : Greeting = Greeting { lang: Lang::Posh, message: String::from("Salutations WasmEdge!")};
  v.push(g);
